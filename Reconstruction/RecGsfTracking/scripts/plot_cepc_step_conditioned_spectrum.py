...
def main():
    ...
    loss_edges = (np.linspace(0.0, 1.0, 121) if args.linear_x else
                  np.logspace(-8, 0, 121))
    ...
    centers = ((loss_edges[:-1] + loss_edges[1:]) / 2.0 if args.linear_x else
    ...
    for index, (axis, knot) in enumerate(zip(axes.flat, knots)):
        ...
        axis.set_ylabel("probability / bin" if args.linear_x else
                        "probability / log bin")

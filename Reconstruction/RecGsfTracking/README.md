...
backward component against the moment-matched forward message at the same
layer. It records the all-other-hits smoothed state, the omitted hit's exact
local innovation likelihood, the corresponding all-hit state, the full
five-dimensional compatibility of the forward-updated and backward-predicted
messages, and normalized scores with and without the existing backward prior.
These quantities never replace live component weights or states. A
...
| CMS-like per-layer smoothing diagnostics | `GSFLineageNodeCmsSmoothValid`, plus the `GSFLineageNodeCmsSmooth*` forward-message, all-other/all-hit state, local-innovation, evidence, and normalized-score collections described below |
...
| `lineage_node_source`, `lineage_node_operation`, `lineage_node_hit_index`, `lineage_node_surface_index`, `lineage_node_component_id`, `lineage_node_generation` | Workflow side, creation operation, call-site hit/surface, diagnostic component ID, and BH generation. Source is `1` forward, `2` reverse/backward, or `3` CMS-like forward×backward combination. Operation is `1` seed, `2` BH split child, `3` evaluated measurement result, `4` KL-merge output, or `5` CMS product candidate. Smoother graphs contain the forward construction used by the smoother; reverse and CMS-like graphs additionally link the forward state to every backward seed. CMS product candidates have one source-1 and one source-2 parent. Their source-2 measurement parent contributes its persisted predicted fields, not its filtered posterior fields. |
...
| `lineage_node_cms_smooth_valid`, `lineage_node_cms_smooth_forward_predicted_kappa`, `lineage_node_cms_smooth_forward_predicted_kappa_variance`, `lineage_node_cms_smooth_forward_updated_kappa`, `lineage_node_cms_smooth_forward_updated_kappa_variance` | Passive CMS-like per-layer smoothing availability and the moment-matched forward messages at the current layer. The predicted message contains accepted inner hits but excludes the current hit; the updated message additionally contains the current hit and reflects the normal forward cutoff/reduction. These fields are finite only on accepted interior CMS backward measurement nodes for which both messages and the exact local measurement linearization are valid. |
| `lineage_node_cms_smooth_all_other_kappa`, `lineage_node_cms_smooth_all_other_kappa_variance`, `lineage_node_cms_smooth_all_other_pT`, `lineage_node_cms_smooth_all_hit_kappa`, `lineage_node_cms_smooth_all_hit_kappa_variance`, `lineage_node_cms_smooth_all_hit_pT` | Products of the component's backward-predicted Gaussian with the forward-predicted and forward-updated moment matches, respectively. `all_other` excludes the current hit; `all_hit` includes it exactly once. pT is `1/abs(kappa)` and is NaN for an unusable kappa. |
| `lineage_node_cms_smooth_all_hit_compatibility_dchi2`, `lineage_node_cms_smooth_all_hit_compatibility_logdet`, `lineage_node_cms_smooth_all_hit_log_overlap` | Full five-dimensional compatibility of the moment-matched forward-updated message and the component's backward-predicted message. For their means `mu_F`, `mu_B` and covariances `C_F`, `C_B`, `dchi2 = (mu_F-mu_B)^T (C_F+C_B)^-1 (mu_F-mu_B)`, `logdet = log(det(C_F+C_B))`, and `log_overlap = -0.5 * (5*log(2*pi) + logdet + dchi2)`. For two sibling hypotheses the compatibility-score difference is the radiative-minus-identity difference of `dchi2+logdet`; its relative likelihood factor is `exp(-0.5*delta_score)`. These passive quantities do not include the backward prior weight. |
| `lineage_node_cms_smooth_identity_compatibility_valid`, `lineage_node_cms_smooth_forward_identity_node_id`, `lineage_node_cms_smooth_forward_identity_weight`, `lineage_node_cms_smooth_forward_identity_updated_kappa`, `lineage_node_cms_smooth_forward_identity_updated_kappa_variance` | Passive identity-controlled CMS compatibility metadata. At each accepted interior hit, the forward reference is the positive-weight `no_radiation` survivor with the largest normalized weight after the normal forward cutoff and reduction. The node ID points to that exact forward lineage node. The same forward identity state is attached to every backward candidate at the hit; `valid=1` additionally requires a finite, positive-definite full-state compatibility calculation. These fields never select or reweight a component. |
| `lineage_node_cms_smooth_identity_compatibility_dchi2`, `lineage_node_cms_smooth_identity_compatibility_logdet`, `lineage_node_cms_smooth_identity_compatibility_log_overlap` | Full five-dimensional compatibility of the fixed forward identity update and the current backward-predicted component, using the same formula as the moment-matched compatibility row above. Comparing an identity and radiative backward sibling now holds the forward side exactly fixed, so only the backward hypothesis changes. The score excludes the backward prior and remains a correlated-filter diagnostic, not a calibrated Bayes factor. |
| `lineage_node_cms_smooth_all_other_log_overlap`, `lineage_node_cms_smooth_local_dchi2`, `lineage_node_cms_smooth_local_logdet_innovation`, `lineage_node_cms_smooth_local_log_likelihood` | Gaussian compatibility of the forward-predicted and backward-predicted messages, followed by the omitted current hit evaluated at the all-other-hits state. For measurement dimension `m`, `local_log_likelihood = -0.5 * (m*log(2*pi) + local_logdet_innovation + local_dchi2)`. The residual/projector and reference innovation are taken from the exact accepted MarlinTrk update. Both terms are anchored to its accepted `dchi2` and `logDetInnovation`, then only their linearized change is transported to the smoothed state; evaluating the original predicted state therefore reproduces the live weighting convention exactly. |
| `lineage_node_cms_smooth_log_evidence`, `lineage_node_cms_smooth_log_weight_with_prior`, `lineage_node_cms_smooth_normalized_evidence`, `lineage_node_cms_smooth_normalized_weight_with_prior` | `log_evidence = all_other_log_overlap + local_log_likelihood`; `log_weight_with_prior` additionally adds the logarithm of the existing backward prior weight entering this hit. The last two fields normalize those two scores independently within each `(input_track_index, hit_index)` candidate group. This exposes whether all-layer compatibility can overcome a small radiative prior without changing that prior or steering the fit. Because the CMS backward seed ultimately derives from the forward fit and the forward message is moment-matched, these are diagnostic relative scores, not calibrated independent Bayes factors. |

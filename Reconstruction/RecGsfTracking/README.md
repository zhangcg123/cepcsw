...
`NextMeasurement` and `NextNextMeasurement` modes make temporary copies of a
newly split inward BH cohort and probe only hit `i-1` or `i-2`, respectively,
for a split on the `i+1 -> i` interval. They transfer the resulting normalized
look-ahead weights back to the children at the split surface, discard the
temporary states, and then resume the ordinary adjacent-hit recursion at hit
`i`. A target beyond the innermost measurement is clamped to hit 0.
...
remain expanded through measurement `i`. For `NextMeasurement` and
`NextNextMeasurement`, temporary copies of those children propagate from the
split surface to `i-1` or `i-2` without intermediate measurement updates or
additional BH splitting. Only the target measurement evaluates the copies.
Its normalized posterior weights are transferred to the original children at
the split surface; the copied states are discarded, and the live children
then follow the unchanged adjacent-hit workflow beginning at measurement `i`.
Negative target indices clamp to hit 0, preserving the terminal hit-0 update:
a split with `i=0` probes hit 0 in either mode, and a split with `i=1` probes
hit 0 in both look-ahead modes. Material intervals crossed by the temporary
probe are processed normally when the live recursion subsequently reaches
them. Because the selected look-ahead hit is encountered again by that live
recursion, its evidence is intentionally counted once in the probe and once
in the ordinary update in this diagnostic implementation. A transition can
temporarily require roughly `MaxComponents * number-of-BH-modes` live
children plus their look-ahead copies.
...
The focused implementation and regression evidence for the two look-ahead
weighting modes is recorded in

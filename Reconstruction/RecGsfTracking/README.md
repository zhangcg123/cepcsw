...
| `lineage_node_fb_delta_kappa`, `lineage_node_fb_delta_kappa_variance`, `lineage_node_fb_delta_pT`, `lineage_node_fb_delta_pT_variance` | Passive forward/backward compatibility diagnostics for each direct source-3 operation-5 pair. The signed definitions are `delta_kappa=kappa(B_predicted)-kappa(F_updated)` and `delta_pT=pT(B_predicted)-pT(F_updated)`, with `pT=1/abs(kappa)`. Because no forward/backward cross-covariance is available, the stored variances use the same independence approximation as the overlap: `Var(delta_kappa)=C_F(kappa,kappa)+C_B(kappa,kappa)` and first-order `Var(delta_pT)=C_F/kappa_F^4+C_B/kappa_B^4`. Values are NaN for other node types, reduced source-3 KL outputs, invalid inputs, and the two boundary mixtures. These fields never steer filtering, weights, reduction, or publication. Older EDM inputs without the four collections retain the lineage graph and receive row-aligned NaNs in the flat tuple. |

...
`LocalMeasurement`, also keeps their weights diagnostic-only. The independent
integer `InwardLookaheadDepth` defaults to zero. A positive depth `N` makes
temporary copies of a newly split inward BH cohort and probes every unique
available inner hit from `i-1` through `i-N` for a split on interval
`i+1 -> i`. Without the beam experiment, targets stop at hit 0. With
`BeamSpotConstraint=true`, the beam boundary follows hit 0 as one additional
target, so depth 1 can probe the beam for a split on hit 1 to hit 0, depth 2
can probe hit 0 and the beam for a split on hit 2 to hit 1, and so on. Each
boundary's posterior is normalized separately, the posteriors are
arithmetically averaged, and the temporary states are discarded. The live
children retain a separate original-BH-prior channel and resume the ordinary
adjacent-hit update at hit `i`.
...
measurement `i`. For positive look-ahead depth `N`, temporary copies of the
reverse children probe the unique hits `max(0,i-d)` for `d=1..N`, without
intermediate measurement updates or additional BH splitting. For BH child
`k`, let `b_k` be its normalized original prior and `L_h(k)` the likelihood at
probe hit `h`. Each probe forms and separately normalizes
`q_h(k) = normalize_k[b_k L_h(k)]`, then the feedback channel is the arithmetic
mean `qbar_k = mean_h[q_h(k)]`. The copies are discarded. The ordinary live
update at hit `i` is performed only once, yielding one updated state and
covariance per child and local likelihood `L_i(k)`. Before cutoff and KL
reduction, the two equal-status unnormalized channels are added:
`u_k = b_k L_i(k) + qbar_k L_i(k)`, followed by one global normalization over
all children. Thus the original BH-prior channel is preserved rather than
replaced by look-ahead feedback. Material intervals crossed by a probe are
processed normally when the live recursion subsequently reaches them. Every
probed hit is also reused by that recursion, so its evidence is deliberately
reused in this diagnostic implementation. Hit-0 clamping preserves the
terminal update, and duplicate clamped targets are evaluated only once. A
transition can temporarily require roughly
`MaxComponents * number-of-BH-modes` live children plus one temporary copy of
...
| `InwardLookaheadDepth` | `0` | `0` | Nonnegative number of unique farther-inward boundaries probed after an actual inward BH split while `InwardWeightMode=LocalMeasurement`. Zero is the ordinary adjacent-only workflow; one reproduces the former `NextMeasurement` target; two includes the former `NextMeasurement` and `NextNextMeasurement` targets before averaging. Real-hit targets descend through hit 0. With `BeamSpotConstraint=true`, the beam spot is the next target after hit 0 and uses the same hit-to-beam propagation and scalar likelihood as the live terminal beam update; without beam mode, probing stops at hit 0. A failed component probe contributes zero at that probe without deleting the live component. A wholly invalid probe is omitted from the average; if none is valid, the feedback vector falls back to the original prior, which reduces to baseline weighting after normalization. A positive value with `SmoothedMarginal` is invalid. The maintained `DumpGsfTrks/gsf.py.bk` reverse branch currently keeps depth `0` for the isolated beam comparison, without changing the compiled or active-template default zero. |
...
The focused implementation and regression evidence for the earlier named
look-ahead modes and their integer-depth successor is recorded in
...
forward writes that state to `GSFTracks`. Every successful smoother or reverse
track is additionally written row-for-row to three collections:
`GSFTracksBestBranch` is that same fixed BestBranch,
...
When `InwardLookaheadDepth` is positive, the beam can also be the final
passive look-ahead target after hit 0. Such a probe starts from the same split
state as every real-hit look-ahead, propagates directly from its current outer
reference to the beam, and evaluates the same scalar likelihood implementation
used by the live terminal update. It performs no extra intermediate BH split,
does not mutate the component, and is averaged with the other valid probe
posteriors. The ordinary recursion still updates hit 0 and subsequently runs
the live hit-0-to-beam split, propagation, beam update, cutoff, and reduction.

...
namespace MarlinTrk {
  ...
  MarlinDDKalTestTrack::MarlinDDKalTestTrack( MarlinDDKalTest* ktest) 
    ...
    //streamlog::out.init(std::cout, "MarlinDDKalTestTrack");
  ...
  int MarlinDDKalTestTrack::addHit(edm4hep::TrackerHit& trkhit) {
    streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::addHit: trkhit = " << trkhit.id() << std::endl;
    return this->addHit( trkhit, _ktest->findMeasLayer( trkhit )) ;

  } 
  
  int MarlinDDKalTestTrack::addHit(edm4hep::TrackerHit& trkhit, const DDVMeasLayer* ml) {

    streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::addHit: trkhit = " << trkhit.id() << " pos: " << trkhit.getPosition() << " ml = " << ml << " " << ml->GetName() << std::endl ;
  ...
  int MarlinDDKalTestTrack::addHit( edm4hep::TrackerHit& trkhit, DDVTrackHit* kalhit, const DDVMeasLayer* ml) {
...
//    Cov.Print();
  ...
  int MarlinDDKalTestTrack::initialise(const edm4hep::TrackState& ts, double /*bfield_z*/, bool fitDirection ) {
    ...
    streamlog_out( DEBUG2 ) << "MarlinDDKalTestTrack::initialise using TrackState: track parameters used for init : " << ts << std::endl;
...
//    covK.Print();
  ...
  int MarlinDDKalTestTrack::addAndFit( DDVTrackHit* kalhit, double& chi2increment, TKalTrackSite*& site, double maxChi2Increment) {
    ...
    if (!_kaltrack->AddAndFilter(*temp_site)) {        
      ...
      std::cout << "Kaltrack::addAndFit : site discarded! at index : " << ml->GetIndex()
		<< " for type " << ml->GetName()
		<< " chi2increment = " << chi2increment
		<< " maxChi2Increment = " << maxChi2Increment
		<< " x = " << pos.x()
  ...
  int MarlinDDKalTestTrack::addAndFit( edm4hep::TrackerHit& trkhit, double& chi2increment, double maxChi2Increment) {
    ...
    if( ml == 0 ){  
      ...
      streamlog_out( ERROR ) << ">>>>>>>>>>>  no measurment layer found for trkhit cellid0 : " 
			     << cellIDString( trkhit.getCellID() ) << " at " 
			     << edm4hep::Vector3d( trkhit.getPosition() ) << std::endl ;
    ...
    if( !_trackHitAtPositiveNDF.isAvailable() && _kaltrack->GetNDF() >= 0){
      ...
      streamlog_out( DEBUG2 ) << ">>>>>>>>>>>  Fit is now constrained at : "
			      << cellIDString( trkhit.getCellID() ) 
			      << " pos " << edm4hep::Vector3d( trkhit.getPosition() )
  ...
  int MarlinDDKalTestTrack::testChi2Increment( edm4hep::TrackerHit& trkhit, double& chi2increment ) {
  ...
  int MarlinDDKalTestTrack::fit( double maxChi2Increment ) {
    ...
    while ( (kalhit = dynamic_cast<DDVTrackHit *>( next() ) ) ) {
      ...
      if( error_code == 0 ){ // add trkhit to map associating trkhits and sites
        ...
        _outlier_chi2_values.push_back(std::make_pair(trkhit, chi2increment));
        streamlog_out( DEBUG2 ) << ">>>>>>>>>>>  fit(): Number of Outliers : " << _outlier_chi2_values.size() << std::endl;
	std::cout << ">>>>>>>>>>>  fit(): Number of Outliers : " << _outlier_chi2_values.size() << std::endl;
   ...
   */
  int MarlinDDKalTestTrack::smooth( edm4hep::TrackerHit& trkhit ) {
    
    streamlog_out( DEBUG2 )  << "MarlinDDKalTestTrack::smooth( edm4hep::TrackerHit& " << trkhit << "  ) " << std::endl ;
  ...
  int  MarlinDDKalTestTrack::getTrackState( edm4hep::TrackState& ts, double& chi2, int& ndf ) {
    
    streamlog_out( DEBUG2 )  << "MarlinDDKalTestTrack::getTrackState( edm4hep::TrackState& ts ) " << std::endl ;
  ...
  int MarlinDDKalTestTrack::getTrackState( edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf ) {
    
    streamlog_out( DEBUG2 )  << "MarlinDDKalTestTrack::getTrackState( edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts ) using hit: " << trkhit << " with cellID0 = " << trkhit.getCellID() << std::endl ;
  ...
  int MarlinDDKalTestTrack::getHitsInFit( std::vector<std::pair<edm4hep::TrackerHit, double> >& hits ) {
...
//    }
  ...
  int MarlinDDKalTestTrack::getOutliers( std::vector<std::pair<edm4hep::TrackerHit, double> >& hits ) {
...
//    }
  ...
  int MarlinDDKalTestTrack::getTrackerHitAtPositiveNDF( edm4hep::TrackerHit& trkhit ) {
  ...
  int MarlinDDKalTestTrack::extrapolate( const edm4hep::Vector3d& point, edm4hep::TrackState& ts, double& chi2, int& ndf ){  
  ...
  int MarlinDDKalTestTrack::extrapolate( const edm4hep::Vector3d& point, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf ) {
  ...
  int MarlinDDKalTestTrack::extrapolate( const edm4hep::Vector3d& point, const TKalTrackSite& site ,edm4hep::TrackState& ts, double& chi2, int& ndf ){  
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolate( const edm4hep::Vector3d& point, edm4hep::TrackState& ts, double& chi2, int& ndf ) called " << std::endl ;
  ...
  int MarlinDDKalTestTrack::extrapolateToLayer( int layerID, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
  ...
  int MarlinDDKalTestTrack::extrapolateToLayer( int layerID, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
  ...
  int MarlinDDKalTestTrack::extrapolateToLayer( int layerID, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolateToLayer( int layerID, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID ) called " << std::endl ;
  ...
  int MarlinDDKalTestTrack::extrapolateToDetElement( int detElementID, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
  ...
  int MarlinDDKalTestTrack::extrapolateToDetElement( int detElementID, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
  ...
  int MarlinDDKalTestTrack::extrapolateToDetElement( int detElementID, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolateToDetElement( int detElementID, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) called " << std::endl ;
  ...
  int MarlinDDKalTestTrack::propagate( const edm4hep::Vector3d& point, edm4hep::TrackState& ts, double& chi2, int& ndf ){
    ...
    const DDVMeasLayer* ml = (  (_ktest->getIPLayer() ) && sqrt(point.x*point.x+point.y*point.y+point.z*point.z) < _ktest->getIPLayer()->GetR()) ? _ktest->getIPLayer() : 0;
    
    return this->propagate( point, site, ts, chi2, ndf, ml ) ;
    
  }
  
  int MarlinDDKalTestTrack::propagate( const edm4hep::Vector3d& point, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf ){
    ...
    if ( ml )
      if (sqrt(point.x*point.x+point.y*point.y+point.z*point.z) > _ktest->getIPLayer()->GetR()) ml = NULL;
...
//    const DDVMeasLayer* ml = (point.r() < _ktest->getIPLayer()->GetR()) ? _ktest->getIPLayer() : 0;
  ...
  int MarlinDDKalTestTrack::propagate( const edm4hep::Vector3d& point, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, const DDVMeasLayer* ml ){
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagate( const edm4hep::Vector3d& point, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf ) called " << std::endl ;
...
//    Double_t cpa = helix.GetKappa();                              // get pt 
  ...
  int MarlinDDKalTestTrack::propagateToLayer( int layerID, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
  ...
  int MarlinDDKalTestTrack::propagateToLayer( int layerID, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
  ...
  int MarlinDDKalTestTrack::propagateToLayer( int layerID, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID, int mode ) { 
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagateToLayer( int layerID, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, int& detElementID ) called " << std::endl;
  ...
  int MarlinDDKalTestTrack::propagateToDetElement( int detElementID, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
  ...
  int MarlinDDKalTestTrack::propagateToDetElement( int detElementID, edm4hep::TrackerHit& trkhit, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
  ...
  int MarlinDDKalTestTrack::propagateToDetElement( int detElementID, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) { 
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagateToDetElement( int detElementID, edm4hep::TrackState& ts, double& chi2, int& ndf, int mode ) called " << std::endl ;
  ...
  int MarlinDDKalTestTrack::intersectionWithDetElement( int detElementID, edm4hep::Vector3d& point, int mode ) {  
  ...
  int MarlinDDKalTestTrack::intersectionWithDetElement( int detElementID,  edm4hep::TrackerHit& trkhit, edm4hep::Vector3d& point, int mode ) {  
  ...
  int MarlinDDKalTestTrack::intersectionWithDetElement( int detElementID, const TKalTrackSite& site, edm4hep::Vector3d& point, const DDVMeasLayer*& ml, int mode ) {
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::intersectionWithDetElement( int detElementID, const TKalTrackSite& site, edm4hep::Vector3d& point, const DDVMeasLayer*& ml, int mode) called " << std::endl;
  ...
  int MarlinDDKalTestTrack::intersectionWithLayer( int layerID, edm4hep::Vector3d& point, int& detElementID, int mode ) {  
  ...
  int MarlinDDKalTestTrack::intersectionWithLayer( int layerID,  edm4hep::TrackerHit& trkhit, edm4hep::Vector3d& point, int& detElementID, int mode ) {  
  ...
  int MarlinDDKalTestTrack::intersectionWithLayer( int layerID, const TKalTrackSite& site, edm4hep::Vector3d& point, int& detElementID, const DDVMeasLayer*& ml, int mode ) {  
    
    streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::intersectionWithLayer( int layerID, const TKalTrackSite& site, edm4hep::Vector3d& point, int& detElementID, int mode) called layerID = " << layerID << std::endl;
    
    std::vector<DDVMeasLayer const*> meas_modules ;
    _ktest->getSensitiveMeasurementModulesForLayer( layerID, meas_modules ) ;  
    
    if( meas_modules.size() == 0 ) {
      std::cout << "MarlinDDKalTestTrack::intersectionWithLayer layer id unknown: layerID = " << layerID << std::endl;
  ...
  int MarlinDDKalTestTrack::findIntersection( const DDVMeasLayer& meas_module, const TKalTrackSite& site, edm4hep::Vector3d& point, double& dphi, int& detElementID, int mode ) {
    ...
    std::cout << "MarlinDDKalTestTrack::intersectionWithLayer crossing_exist = " << crossing_exist << " dphi " << dphi << " with detElementIDs: " << detElementID << std::endl;
  ...
  int MarlinDDKalTestTrack::findIntersection( std::vector<DDVMeasLayer const*>& meas_modules, const TKalTrackSite& site, edm4hep::Vector3d& point, int& detElementID, const DDVMeasLayer*& ml, int mode ) {
  ...
  void MarlinDDKalTestTrack::ToLCIOTrackState( const THelicalTrack& helix, const TMatrixD& cov, edm4hep::TrackState& ts, double& chi2, int& ndf) const {
    ...
    ts.referencePoint = edm4hep::Vector3f(pivot);
  ...
  void MarlinDDKalTestTrack::ToLCIOTrackState( const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf ) const {
  ...
  int MarlinDDKalTestTrack::getSiteFromLCIOHit( edm4hep::TrackerHit& trkhit, TKalTrackSite*& site ) const {

...
private:
   ...
   *  this order will define the direction of the energy loss used in the fit
   */    
  int addHit(edm4hep::TrackerHit& trkhit, const DDVMeasLayer* ml) ;
  
  /** add hit to track - the hits have to be added ordered in time ( i.e. typically outgoing )
   *  this order will define the direction of the energy loss used in the fit
   */    
  int addHit(edm4hep::TrackerHit& trkhit, DDVTrackHit* kalhit, const DDVMeasLayer* ml) ;
   ...
   *  returning TrackState, chi2 and ndf via reference   
   */    
  int propagate( const edm4hep::Vector3d& point, const TKalTrackSite& site, edm4hep::TrackState& ts, double& chi2, int& ndf, const DDVMeasLayer* ml = 0 ) ;
   ...
   *  returning intersection point in global coordinates and integer ID of the intersected sensitive detector element via reference 
   */
  int intersectionWithLayer( int layerID, const TKalTrackSite& site, edm4hep::Vector3d& point, int& detElementID, const DDVMeasLayer*& ml, int mode=modeClosest ) ;
   ...
   *  returning intersection point in global coordinates via reference 
   */
  int intersectionWithDetElement( int detElementID, const TKalTrackSite& site, edm4hep::Vector3d& point, const DDVMeasLayer*& ml, int mode=modeClosest ) ;
   ...
   */
  int findIntersection( std::vector<DDVMeasLayer const*>& meas_modules, const TKalTrackSite& site, edm4hep::Vector3d& point, int& detElementID, const DDVMeasLayer*& ml, int mode=modeClosest ) ;
  
  /** extrapolate the fit at the measurement site, to the DDVMeasLayer,
   *  and return intersection point in global coordinates via reference 
   */
  int findIntersection( const DDVMeasLayer& meas_module, const TKalTrackSite& site, edm4hep::Vector3d& point, double& dphi, int& detElementIDconst, int mode=modeClosest ) ;
   ...
   */
  std::map<edm4hep::TrackerHit,DDVTrackHit*> _lcio_hits_to_kaltest_hits{};

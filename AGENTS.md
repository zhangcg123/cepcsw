...
The user has explicitly selected `MaxComponents=24` as the active default.
The C++ property, documentation, standard electron steering, reverse-template
fallback, and sample helper now agree on 24; the package builds and installs.
A comprehensive-verbose default-helper check on light events 284/1 and 404/8
completed and is identical at tuple precision to the earlier explicit
24-component outputs. Use 24 as the baseline for subsequent comparisons.

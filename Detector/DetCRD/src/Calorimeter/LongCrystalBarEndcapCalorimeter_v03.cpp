    ...
    for(xml_coll_t m(x_det,_U(layer)); m; ++m){  // loop over layers
      ...
      if(module_type == 0 || module_type == 20){ // for cube first place bar then place layer
        ...
        for (int j = 0; j < repeat; j++){
          ...
          double active_layer_dim_x = (module_type == 1  || module_type == 2 || module_type == 21 || module_type == 22) ? trap_half_y1 - EcalEndcap_deadarea_thickness 
            : (module_type == 3 || module_type == 4 || module_type == 5 || module_type == 6 || module_type == 7 || module_type == 8 || module_type == 9 || module_type == 10) ? trap_half_x1/2 - EcalEndcap_deadarea_thickness + j*delta_layer/2 
            : (dim_x+dim_y)/2 - EcalEndcap_deadarea_thickness + j*delta_layer/2; 
          double active_layer_dim_y = (module_type == 1  || module_type == 2 || module_type == 21 || module_type == 22) ? trap_half_x1 - EcalEndcap_deadarea_thickness - j*delta_layer 
          ...
          double active_layer_dim_z = (module_type == 1  || module_type == 2 || module_type == 21 || module_type == 22) ? layer_thickness/2.0 
            : (module_type == 3 || module_type == 4 || module_type == 5 || module_type == 6 || module_type == 7 || module_type == 8 || module_type == 9 || module_type == 10) ? trap_half_y1/2 - EcalEndcap_deadarea_thickness 
            : layer_thickness/2.0;
          ...
          layer_phv = (module_type == 1 || module_type == 2 || module_type == 21 || module_type == 22) ? envelopeVolTrapIso.placeVolume(layer_vol, Transform3D(RotationZYX(rot_layer, 0, 0), Position(0,0,layer_pos_z))) 
            : (module_type>=3 && module_type<=10) ? envelopeVolTrapRight.placeVolume(layer_vol, Transform3D(RotationZYX( 0, 0, 0), Position( -(dim_x2 - dim_x1)/2  + j*delta_layer/2 + EcalEndcap_deadarea_thickness, layer_pos_z, 0))) 
            : (module_type>=11 && module_type<=14) ? envelopeVolPrism.placeVolume(layer_vol, Position(-dim_x + active_layer_dim_x + EcalEndcap_deadarea_thickness, -dim_x + active_layer_dim_y + EcalEndcap_deadarea_thickness, layer_pos_z))
            : envelopeVolPrism.placeVolume(layer_vol, Transform3D(RotationZYX( rot_layer, 0, 0), Position(-dim_x + active_layer_dim_x + EcalEndcap_deadarea_thickness, -dim_x + active_layer_dim_y + EcalEndcap_deadarea_thickness, layer_pos_z)));
